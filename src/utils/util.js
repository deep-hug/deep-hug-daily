// 根据数组中每个对象的某一属性就行排序     使用:  var arr =  [{i: 3}, {i: 1}]    arr.sort(utils.compare('i'))  
// 现在是从小到大排序     如果想要从大到小排序，将 value1 - value2  替换成  value2 - value1 即可
...
// 数组对象去重的方法    使用：  var arr = [{i: 1,d:1}, {i: 2,d: 1}, {i:1, d:2}]    utils.removalRepeat(arr, 'i')
function removalRepeat(array, property) {
  var obj = {};
...
function subtraction(total, ...arr) {
  ...
  if (value >= 0) {    // 这个if判断 value最小值是0（因为是金额的数值运，会减掉优惠券的金额，最小为0）   total是开始的金额，后面的都是应该减掉的金额
    return formatCurrency(value);
  } else {
    return 0;
  }
